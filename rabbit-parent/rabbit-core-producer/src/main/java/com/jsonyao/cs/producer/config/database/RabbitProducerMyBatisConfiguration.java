...
/**
 * 配置Mybatis: SqlSessionFactory, SqlSessionTemplate, 扫描Mapping, 必须在数据源注入后初始化
 */
@Configuration
...
public class RabbitProducerMyBatisConfiguration {
	...
	public SqlSessionFactory rabbitProducerSqlSessionFactory(DataSource rabbitProducerDataSource) {
		...
		try {
			bean.setMapperLocations(resolver.getResources("classpath:com/jsonyao/cs/producer/mapping/*.xml"));
